...
Make quality profiles configurable, not just pickable. The SDK gains `qualityProfile.get`/`create`/`update`/`remove` and a faithfully-modeled profile (its quality `items` tree, `cutoff`, format-score thresholds, `formatItems`, and `language`), plus a read-only `language.list`. An update fetches the current resource, overlays the patch, and PUTs it back, so unmodeled and unspecified fields survive the round-trip.
