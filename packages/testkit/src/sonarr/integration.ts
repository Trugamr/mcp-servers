...
import { bootServiceContainer } from "../container.js"
...
export const createSonarrGlobalSetup =
  ...
  async ({ provide }: TestProject) => {
    ...
    const service = await bootServiceContainer({
      image: IMAGE,
      port: SONARR_PORT,
      apiKey: API_KEY,
      apiKeyEnv: "SONARR__AUTH__APIKEY",
      healthPath: "/ping",
      ...(options.tmpfs ? { tmpfs: options.tmpfs } : {}),
    })

    provide("sonarrBaseUrl", service.baseUrl)
