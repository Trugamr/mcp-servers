...
import { injectSonarr, SONARR_VERSION } from "@trugamr/testkit/sonarr"
...
const callTool = (name: string, args: Record<string, unknown>) =>
  callMcp(endpoint, "tools/call", { name, arguments: args }).then(
...
describe("sonarr-mcp tools over Streamable HTTP, against a live Sonarr", () => {
  ...
  it("list_quality_profiles returns the default profiles (an array GET)", async () => {
    const result = await callTool("list_quality_profiles", {})

    expect(result.isError ?? false).toBe(false)
    expect(result.structuredContent).toBeInstanceOf(Array)
    expect((result.structuredContent as ReadonlyArray<unknown>).length).toBeGreaterThan(0)
  })
  ...
  it("creates, lists, then deletes a tag (a write round-trip)", async () => {
    ...
    const tags = listed.structuredContent as ReadonlyArray<{ readonly id: number }>
    expect(tags.some((entry) => entry.id === tag.id)).toBe(true)
  ...
  it("reports a typed tool error for an unknown series id (isError over the wire)", async () => {
    ...
    expect(JSON.stringify(result)).toContain("SonarrResponseError")
  })
})
