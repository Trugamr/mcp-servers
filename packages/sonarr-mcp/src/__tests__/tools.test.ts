...
describe("library tool handlers", () => {
  ...
  it("list_episodes forwards seriesId and seasonNumber as query params", async () => {
    ...
    await Effect.runPromise(
      run((sonarr) =>
        listEpisodes(sonarr, { filter: { "series.id": { eq: 5 }, "season.number": { eq: 2 } } }),
      ),
    )
...
describe("list_episodes query surface", () => {
  ...
  it("filters by missing and hasAired conveniences", async () => {
    expect(
      ids(
        await Effect.runPromise(
          run((s) => listEpisodes(s, { filter: { "series.id": { eq: 5 }, missing: true } })),
        ),
      ),
    ).toEqual([2, 3])
    expect(
      ids(
        await Effect.runPromise(
          run((s) => listEpisodes(s, { filter: { "series.id": { eq: 5 }, hasAired: true } })),
        ),
      ),
    ).toEqual([1, 2])
    expect(
      ids(
        await Effect.runPromise(
          run((s) =>
            listEpisodes(s, {
              filter: { "series.id": { eq: 5 }, missing: true, hasAired: true },
            }),
          ),
        ),
      ),
  ...
  it("filters by airDate range, sorts by airDate desc, and keeps overview", async () => {
    ...
    const sorted = await Effect.runPromise(
      run((s) =>
        listEpisodes(s, {
          filter: { "series.id": { eq: 5 } },
          sort: [{ field: "airDate", order: "desc" }],
        }),
      ),
    )
